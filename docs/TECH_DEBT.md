...
## 十二、Sprint 10（V4-1 ～ V4-10）评审与复盘 · 2026-09-02

### 12.1 测试与覆盖率

- 后端：**`918 passed / 6 skipped`**（2 条 PG testcontainers 环境跳过 + 4 条显式门禁真实 API 集成测试），总覆盖率 **93.21%**（≥85% 门禁达标）；基线 **784 → 918（+134）**。
- 前端：**Vitest `312 passed`（37 个测试文件）**；`vite build` 通过；基线 **289 → 312（+23）**。
- 执行模式说明：V4 未沿用单条提示词逐任务模式，改用**切片流水线**执行（relay 状态机驱动，每片走 起草 → 执行 → 轮询 → 亲手审核 → 修订 → 提交 六阶段），每个切片均由审核方亲手重跑验收。
- 修订轮次：V4-1 / V4-3 / V4-4 / V4-6 / V4-8 / V4-9 零修订一轮过；V4-2 fix1 一轮；V4-5 三轮（第 3 轮经人工批准，测试断言设计缺陷：SQLite 删行后 id 复用 + 子串碰撞）；V4-7 两轮（两轮均为测试自身缺陷：e2e 缺 `fuse_workout` 调用 / 恢复心率夹具同刻撞车）。
...
V4-10 为本切片（文档同步：TECH_DEBT.md 本节 + 开发提示词手册 Sprint 10 索引），无独立提交、无测试增长。

### 12.3 新增技术债

| 编号 | 事项 | 来源 | 影响 | 建议处理 |
|------|------|------|------|----------|
| T32 | 同 workout 内同名动作共享逐组心率：`workout_set_hr` 按 `(movement_name, set_index)` 匹配，同名动作无法区分 | V4-7 / V4-8 / V4-9 | 真实训记同名动作罕见；如需区分，后续给对齐加动作序号维度 | 真实场景出现时再处理 |
| T33 | 佳明→训记动作类别映射表（`GARMIN_CATEGORY_MAP`）仅 18 类；未识别类别跳过校验按高置信处理 | V4-7 | 遇到佳明识别新类别时补充映射表 | 触发新未识别类别即追加 |
...
1. 佳明 `exerciseSets.startTime` 为 **GMT 时基** ISO 字符串，与 `directTimestamp`（epoch ms GMT）同基，比较时**严禁 +8 小时**；`activityDetailMetrics` 的 metrics 值是 float，HR 可为 null（V4-7）。
2. 既有 `extract_heart_rate_series` 返回的 `t` 是**数组序号**不是时间戳——需要时间序列时不能复用（V4-7 另写 `extract_hr_timeline`）。
3. 训记组次**无独立时间戳**（`time`=组间休息秒数），逐组匹配只能以佳明 ACTIVE 组为时间锚、训记按全局顺序对齐（`done=False` 跳过但保留原位置序号）。
4. 测试夹具勿在同一时间刻注入两个不同 HR 点——并列距离使「最近点」断言不确定（V4-7-fix2 撞车教训）。
5. SQLite 删行后 id 复用：断言旧 id 为 None 会误判；消息断言防子串碰撞（"消息1" vs "消息11"）（V4-5-fix3 教训）。

**评审结论**：V4 十个切片全部完成，验收基线（后端 918+6 / 93.21%，前端 312 / 37 文件 / `vite build` 通过）逐项一致无回归，覆盖率门禁维持；新增技术债 T32 / T33 已登记，保留项 T23 / T24 / T25 / T30 维持原判。**建议打 tag `sprint-10`（留待人工执行，流水线不代打 tag）**。
