...
describe('移动端适配（mobile 参数）', () => {
  ...
  it('mobile：单指标趋势 rotate30/日期裁剪/统一 grid/y 轴 name 保留', () => {
    const option = buildMetricTrendOption(WEIGHTS, '体重', 'kg', { mobile: true })
    expect(option.grid).toEqual({ top: 56, left: 8, right: 8, bottom: 44, containLabel: true })
  ...
  it('mobile：体重×容量双轴图 time 轴用 {MM}-{dd} 模板，右 y 轴保留宽度', () => {
    ...
    expect(option.xAxis.axisLabel).toEqual({ rotate: 30, fontSize: 10, formatter: '{MM}-{dd}' })
    ...
    expect(option.yAxis.every((y) => y.nameTextStyle.fontSize === 10)).toBe(true)
  })
})
